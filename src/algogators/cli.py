...
Running `algogators` with no subcommand launches the full-screen TUI.
...
app = typer.Typer(
    name="algogators",
    help="AlgoGators CLI — a terminal research workbench for the AlgoGators Quantitative Research team.",
...
def main(ctx: typer.Context) -> None:
    ...
    if ctx.invoked_subcommand is None:
        from algogators.tui.app import run as run_tui
...
def _resolve_record(slug: str, version: Optional[str]):
    from algogators.research.storage import get_record, latest_record

    record = get_record(slug, version) if version else latest_record(slug)
    if record is None:
        console.print(f"[error]No research record found for slug {slug!r}.[/error]")
...
def writeup(slug: str, version: Optional[str] = typer.Option(None, help="Specific record version; defaults to latest")) -> None:
    ...
    from algogators.research.backtest import has_backtest_result, load_backtest_result
    from algogators.research.data_stage import load_quality_reports
    from algogators.research.writeup import generate_writeup

    record = _resolve_record(slug, version)
    if not has_backtest_result(record):
        console.print("[error]Run `algogators backtest` before generating a writeup.[/error]")
        raise typer.Exit(code=1)
...
def cache_status() -> None:
    ...
    from algogators.data import cache
...
def compare_matrix(universe: str) -> None:
    ...
    from algogators.analytics.correlation import correlation_matrix
...
def compare_cointegration(sym_a: str, sym_b: str) -> None:
    ...
    from algogators.analytics.cointegration import engle_granger_test
    from algogators.data.provider import AssetClass
    from algogators.data import default_provider
...
def compare_relative(universe: str) -> None:
    ...
    from algogators.charts.terminal_charts import multi_line_chart
